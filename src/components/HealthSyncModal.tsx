...
export default function HealthSyncModal({ userId, onClose }: HealthSyncModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        <Card className="rounded-[32px] border-none bg-white dark:bg-[#2D2D2A] shadow-2xl flex flex-col overflow-hidden max-h-[90vh]">
          <CardHeader className="flex flex-row items-center justify-between p-6 pb-2 shrink-0 border-b border-[#F1F3EE]">
            <div className="flex items-center gap-3">
               ...
               <CardTitle className="text-xl font-serif font-bold text-[#2D2D2A] dark:text-[#F8F7F2]">Sync Apple Health</CardTitle>
            </div>
          ...
          <CardContent className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="space-y-4">
               <div>
                  <h3 className="font-bold text-[#2D2D2A] dark:text-[#F8F7F2] text-sm mb-1">Step 1: Open Shortcuts App</h3>
                  <p className="text-sm text-[#8E8D8A]">Open the Shortcuts app on your iPhone and tap "+" to create a new Shortcut.</p>
               </div>
               <div>
                  <h3 className="font-bold text-[#2D2D2A] dark:text-[#F8F7F2] text-sm mb-1">Step 2: Add Actions in Order</h3>
               ...
               <div className="bg-[#F8F7F2] dark:bg-[#1a1a18] p-4 rounded-[20px] border border-[#E8E6E0] dark:border-[#3D3D3A]">
                 <h3 className="font-bold text-[#2D2D2A] dark:text-[#F8F7F2] text-sm mb-3">Settings for "Get Contents of URL":</h3>
                 
                 <div className="space-y-3 text-sm">
                   <div>
                     <span className="font-bold text-[#8E8D8A] mr-2">URL:</span>
                     <code className="text-[#2D2D2A] dark:text-[#F8F7F2] bg-white dark:bg-[#2D2D2A] px-2 py-1 rounded inline-block w-full break-all shadow-sm ring-1 ring-[#E8E6E0] mt-1">{serverUrl}/api/health-sync</code>
                   </div>
                   <div>
                     <span className="font-bold text-[#8E8D8A] mr-2">Method:</span>
                     <code className="text-[#2D2D2A] dark:text-[#F8F7F2] font-bold">POST</code>
                   </div>
                   <div>
                     ...
                     <pre className="text-xs bg-white dark:bg-[#2D2D2A] p-3 rounded-xl overflow-x-auto text-[#2D2D2A] dark:text-[#F8F7F2] shadow-sm ring-1 ring-[#E8E6E0] whitespace-pre-wrap">
{scriptContent}
                     </pre>
                   </div>
                 </div>
               ...
               <div>
                  <h3 className="font-bold text-[#2D2D2A] dark:text-[#F8F7F2] text-sm mb-1">Step 3: Set Automation</h3>
